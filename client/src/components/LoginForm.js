...
const LoginForm = () => {
  ...
  return (
    <div>
        ...
        <form>
            <input onChange={(e)=> {setName(e.target.value)}} value={name} type="text" className="placeholder:italic placeholder:text-slate-400 block mx-auto my-2 bg-white w-5/6 sm:w-3/6 border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Username..."></input>

            {signUp ? 
            <input onChange={(e)=> {setEmail(e.target.value)}} value={email} type="text" className="placeholder:italic placeholder:text-slate-400 block mx-auto my-2 bg-white w-5/6 sm:w-3/6 border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Email..."></input> : null}

            <input onChange={(e)=> {setPassword(e.target.value)}} value={password} type="password" className="placeholder:italic placeholder:text-slate-400 block mx-auto my-2 bg-white w-5/6 sm:w-3/6 border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Password..."></input>
